 ...
 *
 * <p>An incident reaches the system from a caregiver report, an elder emergency call or a
 * missed check-in. From that moment it always has a named responder and a response
 * countdown; if the countdown expires without anyone taking over, responsibility moves to
 * the next level of the escalation chain (UC-SYS02) and the previous responder's record is
...
public record Incident(
	...
	public static final ZoneId CARELINK_ZONE = ZoneId.of("Asia/Singapore");
	 ...
	 */
	public Incident assignTo(Long newResponderUserId, LocalDateTime respondBy) {
		requireOpenForResponse("assign a responder");
		Objects.requireNonNull(newResponderUserId, "responderUserId");
		Objects.requireNonNull(respondBy, "respondBy");
		return copy(newResponderUserId, severity, Status.OPEN, respondBy, null);
	}
	...
	public Incident claimBy(Long userId) {
		...
		if (status == Status.IN_PROGRESS) {
			throw new BusinessRuleViolation(
					...
					"This incident has already been taken over by responder " + responderUserId);
		}
		requireOpenForResponse("take the incident over");
		return copy(userId, severity, Status.IN_PROGRESS, null, null);
	}
	 ...
	 */
	public Incident changeSeverityTo(Severity newSeverity) {
		Objects.requireNonNull(newSeverity, "severity");
		requireOpenForResponse("change the severity");
		if (newSeverity == severity) {
			throw new BusinessRuleViolation(
					"INCIDENT_SEVERITY_UNCHANGED",
					"The incident is already at severity " + severity);
		}
		return copy(responderUserId, newSeverity, status, respondBy, null);
	}

	/** Closes the incident. Only the responder who took it over may do this. */
	public Incident resolveAt(LocalDateTime resolvedAt) {
		Objects.requireNonNull(resolvedAt, "resolvedAt");
		if (status != Status.IN_PROGRESS) {
			throw new BusinessRuleViolation(
					"INCIDENT_NOT_CLAIMED",
					"An incident must be taken over before it can be resolved; it is " + status);
		}
		return copy(responderUserId, severity, Status.RESOLVED, null, resolvedAt);
	}
	...
	public Incident markUnresolvedEscalated() {
		requireOpenForResponse("mark the incident unresolved");
		return copy(responderUserId, severity, Status.UNRESOLVED_ESCALATED, null, null);
	}
	...
	public boolean awaitingTakeOver() {
		return status == Status.OPEN || status == Status.ACKNOWLEDGED;
	}

	/** True when the response countdown has run out and nobody has taken over. */
	public boolean isOverdue(LocalDateTime now) {
		return awaitingTakeOver() && respondBy != null && !now.isBefore(respondBy);
	}

	/** True once the incident can no longer change hands. */
	public boolean isClosed() {
		return status == Status.RESOLVED || status == Status.UNRESOLVED_ESCALATED;
	}

	public boolean isHandledBy(Long userId) {
		return responderUserId != null && responderUserId.equals(userId);
	}

	// ----------------------------------------------------------------------- helpers ---

	private void requireOpenForResponse(String attemptedAction) {
		if (isClosed()) {
			throw new BusinessRuleViolation(
					"INCIDENT_CLOSED",
					"Cannot %s: the incident is %s".formatted(attemptedAction, status));
		}
	}
